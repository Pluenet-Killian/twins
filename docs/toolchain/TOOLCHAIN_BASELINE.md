...
| Toolset MSVC | v143 | 14.44.35207, compilateur 19.44.35228 | sélectionnée | Toolset associé à Visual Studio 2022 ; ABI et intégration Kit connues. |
...
| Triplet Windows | `x64-windows-static-md-v143` | configuré et compilé | sélectionnée | Triplet du dépôt : bibliothèques statiques, runtime MSVC dynamique et toolset v143 explicite pour empêcher toute dérive vers Visual Studio 2026. |
...
Le moteur utilisera `CMAKE_CXX_STANDARD=20`, `CMAKE_CXX_STANDARD_REQUIRED=ON` et `CMAKE_CXX_EXTENSIONS=OFF`. CMake est téléchargé depuis l'archive officielle, vérifié par SHA-256 et exécuté depuis `.tools` par le wrapper du dépôt afin qu'une version présente plus tôt dans le `PATH` ne change pas le build. Les dépendances vcpkg seront déclarées uniquement dans le mode manifeste ; aucun `vcpkg install` global ne constituera une preuve de reproductibilité. Le triplet overlay `cmake/triplets/x64-windows-static-md-v143.cmake` fait partie du contrat de build : la liaison statique évite l'export de types STL aux frontières des DLL natives, tandis que le runtime MSVC reste dynamique. `CMakePresets.json` impose un buildtree court sous `.tools/vcpkg-b` parce que les sources générées de gRPC dépassent sinon la limite de chemin acceptée par MSVC ; ce chemin reste local, reproductible et hors Git. Le build de l'application Kit continuera d'utiliser `repo.bat` et les outils gérés par le Kit App Template lorsque NVIDIA l'exige.

Protobuf 6.33.4 port-version 2, gRPC 1.81.1 et libpqxx 8.0.2 sont sélectionnés et compilés par le premier squelette vertical. Protobuf génère les contrats d'identité et d'état, gRPC transporte le flux d'état calculé entre le moteur C++ et Kit, et libpqxx lit l'actif canonique dans PostgreSQL. Les prochaines dépendances C++ prévues sont mp-units 2.5.0 pour les quantités et OpenTelemetry C++ 1.28.0 pour l'observabilité. Elles restent **candidates** et ne doivent être ajoutées qu'au moment où un composant réel les utilise.
...
| gRPC C++ | 1.81.1 | compilé par vcpkg avec MSVC v143 | sélectionnée | Serveur callback non bloquant du flux d'état interne. |
| gRPC Python Kit | 1.81.0 | roue CPython 3.12 vérifiée et smoke C++–Kit réussi | sélectionnée | Client Kit ; la version mineure correspond au monorepo gRPC et le patch C++ 1.81.1 est spécifique au port natif. |
| Protobuf Python Kit | 6.33.5 | roue ABI3 vérifiée et chargée par Kit | sélectionnée | Runtime compatible avec les stubs Python générés par le plugin Protobuf v32.1. |
| typing-extensions Kit | 4.15.0 | roue universelle vérifiée | sélectionnée | Dépendance transitive explicite de grpcio pour une installation Kit réellement hors ligne. |
...
La génération Protobuf suit une seule configuration `buf.yaml` et `buf.gen.yaml`. Le plugin Python Protobuf v32.1 et le plugin gRPC Python v1.81.0 sont verrouillés ; les stubs générés et versionnés sous l'extension Kit ne sont jamais modifiés manuellement. CMake appelle exclusivement `protobuf::protoc` et `gRPC::grpc_cpp_plugin` fournis par le manifeste vcpkg pour le C++. Les versions des runtimes C++ et Python peuvent différer, mais la combinaison actuelle est validée par un échange C++–Python réel. Aucun agent ne doit appeler un `protoc` trouvé arbitrairement dans le `PATH`.

## Omniverse, OpenUSD et rendu

| Composant | Version cible | État local | Statut | Justification |
|---|---:|---:|---|---|
| Omniverse Kit SDK | 110.2.0 | build et lancement RTX réussis | sélectionnée pour le socle | Le SDK `110.2.0+feature.342835.698af100.gl` résolu par le template officiel construit l'application minimale, ouvre la scène de référence et sélectionne son prim canonique. |
| Kit App Template | `483e364a4176f102f2d3c3aaf9f301a103d61d69` | clone détaché dans `.tools`, recette rejouable | sélectionnée | Commit officiel `main` vérifié le 1er août 2026, dont le changelog déclare Kit 110.2.0 ; `tools/kit.ps1` contrôle le commit, le consentement local et le scaffold. |
| Extensions Kit | verrou généré par Kit 110.2 | résolu dans `twins.engineering.kit` | sélectionnée pour le socle | Le verrou produit par le registre officiel accompagne la configuration source ; toute extension supplémentaire exige un usage et un smoke test. |
| OpenUSD dans Kit | version embarquée par Kit 110.2 | résolue par le SDK Kit | sélectionnée par héritage | Ne jamais remplacer indépendamment les bibliothèques USD internes de Kit. |
...
La fondation C++, Python, Web, contrats et données possède désormais ses manifests, verrous et premières preuves. Toutes les versions sélectionnées du socle local passent le contrôle exact de toolchain et le moteur compile avec MSVC v143 et le Windows SDK retenu. L'application `twins.engineering` issue du Kit App Template officiel est scaffoldée et son build release Kit 110.2 réussit sur le poste de référence. Le smoke visuel du 1er août 2026 a ouvert `assets/usd/reference-rack.usda`, vérifié l'UUID `00000000-0000-4000-8000-000000000001`, sélectionné `/World/Assets/ReferenceRackA01` et rendu la baie avec RTX Real-Time 2.0. La capture observait environ 2,5 Gio de VRAM, 7,9 Gio de mémoire de processus et 113 images par seconde à 1 280 × 783 ; ces valeurs décrivent ce smoke et ne constituent pas encore un benchmark reproductible. Le smoke interprocessus suivant a servi depuis le C++ un run synthétique immuable par gRPC, reçu les séquences 1 à 3 dans Kit et appliqué `OFF`, `STARTING`, puis `RUNNING` aux temps virtuels 0, 2 et 5 secondes dans la session layer OpenUSD. Le fichier USDA source est resté inchangé. Un second lancement sans serveur a produit une attente bornée sans bloquer Kit, puis a repris le flux et rejoué les mêmes séquences après démarrage du serveur. Cette preuve valide la frontière de transport, la reprise et la projection visuelle, pas encore un modèle physique ni l'ensemble de la matrice de performance Kit.
